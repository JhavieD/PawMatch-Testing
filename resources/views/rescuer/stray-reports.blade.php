...
@push('styles')
    <link rel="stylesheet" href="{{ asset('css/rescuer/stray-reports.css') }}">
@endpush
    ...
    <div class="content-wrapper">
        ...
        <div class="top-bar">
            <div class="welcome-section">
                <h1>Stray Reports</h1>
                <p>Review stray animal reports in your area</p>
            </div>
            <div class="profile-section">
                <div class="profile-img">
                    <img src="{{ auth()->user()->profile_image ?? asset('images/default-profile.png') }}" alt="Rescuer Profile">
                </div>
                <div class="profile-info">
                    <strong>{{ auth()->user()->first_name ?? 'Rescuer' }}</strong>
                </div>
            </div>
        </div>
        ...
        <div class="content-card">
            <div class="search-filter">
                <form method="GET" action="{{ route('rescuer.stray-reports') }}" class="search-form">
                    ...
                    @if(request('search') || request('status'))
                        <a href="{{ route('rescuer.stray-reports') }}" class="btn btn-secondary">
        ...
        <div class="report-grid">
            @forelse($reports as $report)
                ...
                <div class="report-card {{ !$report->is_read ? 'unread' : '' }}"
                    ...
                    data-reporter="{{ $report->reporter_name ?? 'Unknown' }}"
...
function markAsRead(reportId) {
    fetch(`/rescuer/stray-reports/${reportId}/mark-read`, {
        method: 'POST',
        headers: {
...
function resolveReport(resolution) {
    ...
    fetch(`/rescuer/stray-reports/${reportId}/resolve`, {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
...
function returnReportToPending() {
    ...
    btns.forEach(btn => btn.disabled = true);
    fetch(`/rescuer/stray-reports/${reportId}/return-pending`, {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Content-Type': 'application/json',
