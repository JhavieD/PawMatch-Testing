...
@section('page-subtitle', 'Manage users, view statistics, and perform actions like banning or deleting users.')
                    ...
                    @forelse($users as $user)
                        <tr>
                            ...
                            <td class="user-role-cell">
                                <span
                                    class="user-role user-role-{{ strtolower($user->role) }}">{{ ucfirst($user->role) }}</span>
                            </td>
                            ...
                            <td class="user-actions">
                                @if ($user->user_id !== auth()->user()->user_id)
                                    ...
                                    <button class="user-action-btn ban" title="Ban User"
                                        onclick="userAction({{ $user->user_id }}, 'ban')">
                            ...
                            <td colspan="6" class="user-table-empty">
                                ...
                                @if (request()->hasAny(['search', 'role', 'status']))
                                    <div class="user-table-clear">
                                        <a href="{{ route('admin.users') }}" class="btn btn-outline">Clear
                                            Filters</a>
                                    </div>
                                @endif
                            </td>
            ...
            <form id="banUserForm" onsubmit="handleBanUser(event)">
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="banReason" name="reason" rows="4" required
                            placeholder="Enter the reason for banning this user..."></textarea>
                    </div>
                </div>
...
@push('scripts')
    <script>
        ...
        function viewUser(userId) {
            fetch(`{{ url('admin/users') }}/${userId}`)
                ...
                .then(data => {
                    ...
                    document.getElementById('viewUserRole').textContent = data.role.charAt(0).toUpperCase() + data.role
                        .slice(1);
                    document.getElementById('viewUserJoined').textContent = new Date(data.created_at)
                        .toLocaleDateString();
        ...
        function showToast(message, type = 'success') {
            ...
            toast.className =
                `px-4 py-3 rounded shadow text-white font-semibold flex items-center space-x-2 animate-fadeIn ${type === 'success' ? 'bg-green-600' : 'bg-red-600'}`;
